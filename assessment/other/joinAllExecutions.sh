	...
	for objective in $objectives; do 
		...
		for nomeSaida in $entradas; do
			...
			for ((run=1;run<31;run++)); do
				...
				file=results/$alg/split/$nameBase$problem-$objective
				cat $file""_fronts.$run.txt >> results/$alg/$nameBase$problem-$objective""_fronts.txt
				cat $file""_solutions.$run.txt >> results/$alg/$nameBase$problem-$objective""_solutions.txt
