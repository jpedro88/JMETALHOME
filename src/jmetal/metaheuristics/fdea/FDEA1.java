...
public class FDEA1 extends Algorithm{
...
<<<<<<< HEAD
	private double[] crowdingDistances;
=======
>>>>>>> a64979edfe8013e57a9e2901e97fb3bcf603526a
	int generations_;
...
<<<<<<< HEAD
	public void calculateCrowdingDistance(SolutionSet population) {
		...
		int numberOfObjectives = population.get(0).numberOfObjectives();
		for (int i = 0; i < size; i++) {
			population.get(i).setCrowdingDistance(0.0);
		}

		for (int i = 0; i < numberOfObjectives; i++) {
			// Ordena a população pelo i-ésimo objetivo
			population.sort(new ObjectiveComparator(i));

			// Define a distância infinita para os extremos
			...
			// Calcula a distância para os intermediários
			for (int j = 1; j < size - 1; j++) {
				double distance = population.get(j + 1).getObjective(i) - population.get(j - 1).getObjective(i);
				distance += population.get(j).getCrowdingDistance();
				population.get(j).setCrowdingDistance(distance);

//				crowdingDistances[j] = distance;
			}
		}
	}
=======
>>>>>>> a64979edfe8013e57a9e2901e97fb3bcf603526a
	public void reproduction(int G, int Gmax) throws JMException{
		...
		calculateCrowdingDistance(population_);
		//chamada do método
		for (int i = 0; i < (populationSize_); i++) {
			if (G < Gmax) {
				// obtain parents
