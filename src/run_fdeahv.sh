...
for objective in $objectives; do
	for problem in $problems; do
      ...
      echo $problem" - "$objective" - "$run
			java jmetal.metaheuristics.fdea.FDEA_main_hv $problem $objective
		 	mv FUN0HV "results/fdeahv-$problem-$objective""_solutions.$run.txt"
			mv VAR0HV "results/fdeahv-$problem-$objective""_fronts.$run.txt"
